...
public:
    ...
    sensor_msgs::msg::Imu createImu(const dss::DSSIMU& imu_msg) {
        ...
        // Covariance - set diagonal elements (required for Cartographer 3D)
        // Small values indicate high confidence in the measurements
        imu.orientation_covariance[0] = 0.01;  // roll variance
        imu.orientation_covariance[4] = 0.01;  // pitch variance
        imu.orientation_covariance[8] = 0.01;  // yaw variance
...
public:
    DSSToROSIMUNode() : Node("DSSToROSIMUNode") {
        ...
        // Publisher를 먼저 생성 (NATS 콜백에서 사용하므로 순서 중요)
